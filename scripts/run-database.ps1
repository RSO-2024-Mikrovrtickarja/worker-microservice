...
Write-Log -Name "Runner" -Content "Using pg_ctl at $PostgresPgCtlBinary"
...
Write-Log -Name "Runner" -Content "Starting PostgreSQL in the background (using pg_ctl start)."
Invoke-Expression "$PostgresPgCtlBinary start -D `"$DatabaseDataDirectory`" -l `"$LogFilePath`" -o `"-p 5433`""
